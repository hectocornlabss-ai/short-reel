...
const vendor: VendorConfig = {
  ...
  description: "เชื่อมต่อแพลตฟอร์มมัลติโมดัล AtlasCloud เข้ากับ Toonflow ค่าเริ่มต้นตั้งตามเอกสารทางการสำหรับพาธข้อความ, ภาพ, วิดีโอ และการตรวจสอบสถานะงาน",
  inputs: [
    { key: "apiKey", label: "API Key", type: "password", required: true, placeholder: "AtlasCloud API Key" },
    { key: "chatBaseUrl", label: "ที่อยู่พื้นฐานข้อความ", type: "url", required: true, placeholder: "https://api.atlascloud.ai/v1", disabled: true },
    { key: "mediaBaseUrl", label: "ที่อยู่พื้นฐานสื่อ", type: "url", required: true, placeholder: "https://api.atlascloud.ai/api/v1", disabled: true },
  ],
  inputValues: {
    apiKey: "",
...
const buildAtlasVideoPayload = (config: VideoConfig, model: VideoModel) => {
  ...
  if (kind === "wanReferenceToVideo") {
    if (imageRefs.length < 1) {
      throw new Error(`${model.name} ต้องการภาพอ้างอิงอย่างน้อย 1 ภาพ`);
    }
...
const imageRequest = async (config: ImageConfig, model: ImageModel): Promise<string> => {
  ...
  logger(`[AtlasCloud รูปภาพ] ส่งงาน: ${model.modelName} -> ${resolvedModelName}, refs=${imageRefs.length}`);
  const submitResp = await axios.post(url, body, { headers });
  const submitData = submitResp.data;

  // การตอบกลับแบบซิงค์ (ได้ภาพทันที)
  ...
  // การตอบกลับแบบอะซิงค์ (ได้ taskId แล้วตรวจสอบสถานะ)
  const taskId = extractTaskId(submitData);
  if (!taskId) {
    throw new Error(`ส่งงานสร้างภาพล้มเหลว: ไม่พบ Task ID การตอบกลับดิบ: ${JSON.stringify(submitData).slice(0, 500)}`);
  }
  ...
  const pollResult = await pollTask(
    async (): Promise<PollResult> => {
      ...
      if (["succeeded", "success", "done", "completed"].includes(status)) {
        ...
        return { completed: true, error: "งานสำเร็จแต่ไม่พบที่อยู่ผลลัพธ์" };
      }
      if (["failed", "error", "cancelled", "canceled", "expired"].includes(status)) {
        return { completed: true, error: extractError(data) || "สร้างภาพล้มเหลว" };
      }
  ...
  if (!pollResult.data) throw new Error("สร้างภาพล้มเหลว: การตรวจสอบสถานะไม่พบข้อมูล");
...
const videoRequest = async (config: VideoConfig, model: VideoModel): Promise<string> => {
  ...
  logger(`[AtlasCloud วิดีโอ] ส่งงาน: ${model.modelName}, ${summary}`);
  ...
  if (!taskId) {
    const syncUrl = extractUrl(submitData);
    if (syncUrl) return await urlToBase64(syncUrl);
    throw new Error(`ส่งงานสร้างวิดีโอล้มเหลว: ไม่พบ Task ID การตอบกลับดิบ: ${JSON.stringify(submitData).slice(0, 500)}`);
  }
  ...
  const pollResult = await pollTask(
    async (): Promise<PollResult> => {
      ...
      if (["succeeded", "success", "done", "completed"].includes(status)) {
        const mediaUrl = extractUrl(data);
        if (mediaUrl) return { completed: true, data: mediaUrl };
        return { completed: true, error: "งานสำเร็จแต่ไม่พบที่อยู่วิดีโอ" };
      }
      if (["failed", "error", "cancelled", "canceled", "expired"].includes(status)) {
        return { completed: true, error: extractError(data) || "สร้างวิดีโอล้มเหลว" };
      }
      return { completed: false };
    },
    5000,
...
const ttsRequest = async (_config: TTSConfig, _model: TTSModel): Promise<string> => {
  // เวอร์ชันปัจจุบันของ AtlasCloud ยังไม่รองรับ TTS
  return "";
};
...
const checkForUpdates = async (): Promise<{ hasUpdate: boolean; latestVersion: string; notice: string }> => {
  return {
    ...
    notice: "AtlasCloud MASS ฉบับร่างแรก",
  };
};
