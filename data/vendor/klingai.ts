...
const vendor: VendorConfig = {
  ...
  name: "KlingAI",
  description:
    "การสร้างวิดีโอด้วย KlingAI\n\nรองรับโมเดลวิดีโอ KlingAI ทุกรุ่น รวมถึง kling-video-o1, kling-v3-omni, kling-v3, kling-v2-6, kling-v2-5-turbo, kling-v2-1, kling-v2-master, kling-v1-6, kling-v1-5, kling-v1 และอื่นๆ\n\nต้องขอ Access Key และ Secret Key ที่[KlingAI Open Platform](https://klingai.com)",
  inputs: [
    { key: "accessKey", label: "Access Key", type: "password", required: true, placeholder: "กรุณากรอก Access Key ของ KlingAI" },
    { key: "secretKey", label: "Secret Key", type: "password", required: true, placeholder: "กรุณากรอก Secret Key ของ KlingAI" },
    { key: "baseUrl", label: "ที่อยู่คำขอ (Base URL)", type: "url", required: true, placeholder: "ค่าเริ่มต้น: https://api-beijing.klingai.com" },
  ],
...
const submitAndPoll = async (submitUrl: string, queryUrlBase: string, requestBody: any): Promise<string> => {
  ...
  logger(`เริ่มส่งงานสร้างวิดีโอ KlingAI: ${submitUrl}`);
  logger(
    `พารามิเตอร์คำขอ: ${JSON.stringify({
      ...requestBody,
  ...
  if (submitResp.data.code !== 0) {
    throw new Error(`ส่งงานล้มเหลว: ${submitResp.data.message || JSON.stringify(submitResp.data)}`);
  }

  const taskId = submitResp.data.data.task_id;
  logger(`ส่งงานสำเร็จแล้ว, Task ID: ${taskId}`);
  ...
  const result = await pollTask(
    async () => {
      ...
      if (queryResp.data.code !== 0) {
        return { completed: true, error: `ตรวจสอบสถานะงานล้มเหลว: ${queryResp.data.message}` };
      }

      const taskData = queryResp.data.data;
      const status = taskData.task_status;
      logger(`กำลังตรวจสอบสถานะ... สถานะงาน: ${status}`);

      if (status === "succeed") {
        const videoUrl = taskData.task_result?.videos?.[0]?.url;
        if (!videoUrl) {
          return { completed: true, error: "งานเสร็จสิ้นแต่ไม่พบ URL ของวิดีโอ" };
        }
        return { completed: true, data: videoUrl };
      }

      if (status === "failed") {
        return { completed: true, error: `สร้างวิดีโอล้มเหลว: ${taskData.task_status_msg || "ข้อผิดพลาดที่ไม่ทราบสาเหตุ"}` };
      }
...
const textRequest = (model: TextModel, think: boolean, thinkLevel: 0 | 1 | 2 | 3) => {
  throw new Error("KlingAI ไม่รองรับโมเดลข้อความ");
};

const imageRequest = async (config: ImageConfig, model: ImageModel): Promise<string> => {
  throw new Error("KlingAI ไม่รองรับโมเดลรูปภาพ");
};

const videoRequest = async (config: VideoConfig, model: VideoModel): Promise<string> => {
  if (!vendor.inputValues.accessKey) throw new Error("ไม่พบ Access Key");
  if (!vendor.inputValues.secretKey) throw new Error("ไม่พบ Secret Key");
  ...
  if (isOmniModel) {
    ...
    if (isSingleImage && imageRefs.length > 0) {
      ...
      if (!requestBody.prompt) requestBody.prompt = "สร้างวิดีโอจากภาพ";
      ...
      if (!requestBody.prompt) requestBody.prompt = "สร้างวิดีโอเปลี่ยนฉากจากภาพเฟรมแรกและเฟรมสุดท้าย";
      ...
      if (!requestBody.prompt) {
        ...
        requestBody.prompt = `สร้างวิดีโอโดยอ้างอิงจาก ${refs}`;
      }
    }
    ...
    if (!hasImageInput) {
      ...
      if (!requestBody.prompt) throw new Error("โหมดสร้างวิดีโอจากข้อความต้องระบุพรอมต์");
    }
  ...
  if (hasMultiRef && imageRefs.length > 0) {
    ...
    const requestBody: any = {
      ...
      prompt: config.prompt || "สร้างวิดีโอจากภาพอ้างอิง",
      mode: mode,
      duration: String(config.duration),
  ...
  if (isText) {
    if (!config.prompt) throw new Error("โหมดสร้างวิดีโอจากข้อความต้องระบุพรอมต์");
  ...
  if ((isSingleImage || isStartEndRequired || isEndFrameOptional || isStartFrameOptional) && imageRefs.length > 0) {
    const requestBody: any = {
      model_name: modelName,
      prompt: config.prompt || "สร้างวิดีโอจากภาพ",
      mode: mode,
      duration: String(config.duration),
      sound: config.audio === true ? "on" : "off",
    };
  ...
  throw new Error("โหมดสร้างวิดีโอนี้ไม่รองรับ หรือขาดพารามิเตอร์ที่จำเป็น");
};
