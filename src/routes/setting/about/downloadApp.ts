...
export default router.post(
  ...
  async (req, res) => {
    ...
    if (reinstall) {
      res.status(200).send(success("กรุณาดาวน์โหลดและติดตั้งเวอร์ชันล่าสุดด้วยตนเองผ่านเบราว์เซอร์"));
    } else {
      ...
      res.status(200).send(success(`อัปเดตเป็น ${version} สำเร็จ กำลังรีสตาร์ทใน 5 วินาที`));
    }
  },
);
