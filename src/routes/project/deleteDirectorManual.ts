...
// 删除导演手册 — แอดมินลบได้ทุกอัน ผู้ใช้ทั่วไปลบได้เฉพาะเทมเพลตที่ตัวเองสร้าง
export default router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!(await canDeleteSkill("story_skills", name, currentUser.id, currentUser.isAdmin))) {
      ...
      await removeOwnership("story_skills", name);
